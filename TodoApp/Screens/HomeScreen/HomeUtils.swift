                        ...
                        Button("Update") {
                            Task{
                                if let token = token{
                                    ...
                                    await todHandle.TodoOperation(url: "https://todo-xiii.onrender.com/api/todo/v1/Todos/todo/update", data: data, token: token)
                                }else{
                    ...
                    .swipeActions(content:{
                        Button(action: {
                            ...
                            Task{
                                ...
                                if let token = token{
                                    await todHandle.TodoOperation(url: "https://todo-xiii.onrender.com/api/todo/v1/Todos/todo/delete", data: data, token: token)
                                }else{
                                    print("tool bar Problem in Login Token")
                                }
                            }
                ...
                Button("Add") {
                    Task{
                        if let token = token{
                            ...
                            await todHandle.TodoOperation(url: "https://todo-xiii.onrender.com/api/todo/v1/Todos/todo/add", data: data, token: token)
                        }else{
                            print("tool bar Problem in Login Token")
                        }
                    }
